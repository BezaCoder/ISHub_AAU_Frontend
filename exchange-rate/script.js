...
function calculate() {
  const currencyElement_one = curreny_one.value;
  const currencyElement_two = curreny_two.value;

  fetch("https://open.exchangerate-api.com/v6/latest")
    .then((res) => res.json())
    .then((data) => {
      const rate =
        data.rates[currencyElement_two] / data.rates[currencyElement_one];
      rateEl.innerText = `1 ${currencyElement_one} = ${rate} ${currencyElement_two}`;
